...
function getPermutations(argstypes)
    expanded = [[c.name for c in argstypes]]
    for idx in range(1, length=length(argstypes))p
        for type in argstypes[idx].superclasses
            expanded = [expanded..., expand(
                            idx
                            , [argstypes[1:idx-1]..., type, argstypes[idx+1:end]...])...]
            [println(p) for p in expanded]
            println("--------")
        end
    end
    return expanded
...
function permute(types, l, r)
    if l == r
        println("l: ", l, " - r: ", r, " types: ", types)
    else
        for i in range(l, length=r-1)
            println("l: ", l, " - r: ", r, " idx: ", i)
            types = swap(types, l, i)
            permute(types, l + 1, r)
            types = swap(types, l, i)
        end
    end
...
function getEffectiveMethod(methods, args...)
    ...
    #println("#########")
    #[println(p) for p in getPermutations(argstypes)]
    #println("#########")
